...
from .utils import which_equivalence, interpolate2d
...
class TheoryCalc(Tables, Result):
    ...
    def process_condition(self, channel_key, condition_dict, parameter, model):
        index = ''
    ...
    def get_channels_from_limit_condition(self, k, width_ratio, coupling, model, relative_width_keys, coupling_keys):
        channel = self.process_condition(k, relative_width_keys, width_ratio, model)
        if channel != '':
            return channel
        channel = self.process_condition(k, coupling_keys, coupling, model)
        return channel

    def get_channel(self, k, r, kappa):
        if self.VLB:
            channel = self.get_channels_from_limit_condition(k, r, kappa, self.m.model(), B_width_mass_ratio_keys,
                                                             B_kappa_keys)
            if self.m.model() == 'Singlet' or self.m.model() == 'Doublet':
                return channel

        elif self.VLX:
            channel = self.get_channels_from_limit_condition(k, r, kappa, self.m.model(), X_width_mass_ratio_keys,
                                                             X_kappa_keys)
            return channel
        elif self.VLY:
            channel = self.get_channels_from_limit_condition(k, r, kappa, self.m.model(), Y_width_mass_ratio_keys,
                                                             Y_kappa_keys)
            return channel
        else:
            if self.m.model() == 'Singlet':
                channel = self.get_channels_from_limit_condition(k, r, kappa, self.m.model(), T_width_mass_ratio_keys,
                                                                 T_kappa_keys)
                return channel
    ...
    def numerator(self, i):
        def pure_VLQ_decay():
            if self.to_W:
                ...
                raise Exception("Error, final state must be indicated")
        ...
        if self.VLB:
            ...
            if self.m.model() == 'Singlet':
                ...
                return pure_VLQ_decay()

        elif self.VLX:
            j = self.get_channel(self.key[i], r, kappa)
            if j == 14:
                return self.m.get_xs_pp_Xtq_twtq(j)
            else:
                return self.m.get_xs_pp_QQ()
            ...
            if self.m.model() == 'Doublet':
                if j in [0, 4, 6, 7]:
                    return self.m.get_xs_pp_Ybq_Wbbq(j)
                elif j == 5:
                    return -1
                else:
                    return self.m.get_xs_pp_QQ()
            if self.m.model() == 'Triplet':
                if j == 0:
                    return self.m.get_xs_pp_Ybq_Wbbq(j)
                else:
                    return self.m.get_xs_pp_QQ()
            ...
            if self.m.model() == 'Singlet':
                ...
                if self.m.get_which_doublet() == 'TB' or self.m.get_which_doublet() == 'XT':
                    ...
                    if j in [23, 24, 25, 26, 27, 28]:
                        ...
                        return self.m.get_xs_pp_Ttq_tHtq(j)

                    elif j in [21, 7, 11, 12, 17, 18]:
                        return self.m.get_xs_pp_Ttq_tZtq(j)

                    elif j in [13, 14, 19, 20]:
                        return self.m.get_xs_pp_Ttq_tZ_plus_TH(j)
            ...
            if 0 <= num:
                if self.VLB:
                    ...
                    if min(self.MX[index]) <= self.m.get_mX() <= max(self.MX[index]):
                        ...
                        expt_xs = expected_or_observed(self.m.get_mX())
                        return expt_xs
                    else:
                        return -1
                elif self.VLY:
                    if min(self.MY[index]) <= self.m.get_mY() <= max(self.MY[index]):
                        ...
                        expt_xs = expected_or_observed(self.m.get_mY())
                        return expt_xs
                    else:
                        return -1
                else:
